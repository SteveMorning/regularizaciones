...
from 
(
SELECT ID_Elemento_ACTUAL as IdElemento, Usuario as usuario ,  if(Usuario = '" .  $_SESSION['id']  . "',0, 1) as userlocal  ,date_format(FECHA_INICIO , '%d/%m/%y %H:%m') as Fecha_Inicio, FECHA_ULTIMA_MODIFICACION as Fecha_Fin, DATEDIFF ( curdate(), FECHA_ULTIMA_MODIFICACION)as dias, 'tomado' as Gestion ,null as 'Tkts Vinculados', null as	Observaciones ,  '' as TIPO_ELEMENTO  FROM bd3_gestiones.gestiones_actuales_elementos   WHERE HERRAMIENTA = 'concentraciones_ICD' 
UNION ALL 
Select ele.ID_Elemento , ele.USUARIO as usuario , 2 as  userlocal, date_format(ele.FECHA_INICIO , '%d/%m/%y %H:%m') as Fecha_Inicio, date_format(ele.FECHA_GESTION , '%d/%m/%y %H:%m') as Fecha_Fin , DATEDIFF ( curdate(), FECHA_GESTION) as dias,  itm.TEXTO_A_MOSTRAR as Gestion  , ele.TICKETS_ALCANZADOS as 'Tkts Vinculados', ele.OBSERVACIONES as Observaciones ,TIPO_ELEMENTO
