...
if ($_SESSION['id'] == '') {  /* ############ VERIFICA SI EL USUARIO ESTA LOGUEADO ################## */
    ...
    if (empty($_POST['idElemento'])) {  /* ############ VERIFICA SI SE SELEECIONO ELEMENTO ################## */
        ...
        if ($cant != 0 ) {    /* ############ SI ESTA TOMADO DEVUELVE QUIEN LO TOMO ################## */
            ...
            // $consulta = "SELECT  Elemento as IdElemento , '" . $_SESSION['id'] . "' as usuario ,  0 as userlocal , DATE_FORMAT( now() , '%d/%m/%y %H:%i') as Fecha_Inicio , null as Fecha_Fin , 0 as dias ,  'tomado' as Gestion,  Pendiente_Total as 'Tkts Vinculados'  , cinum as Observaciones , Tipo_Elemento  , '" . $_SESSION['user'] . "'   as  Colaborador, null as email
